          ...
          <div className="w-[100%]">
            ...
            <div className="space-y-2">
              {Object.entries(RouteIcons).map(
                ([path, { icon: Icon, name }]) => name.includes("New") ? (<a
                  key={path}
                  href={path}
                  className={`relative flex flex-col items-center justify-center space-y-1 p-2 rounded-lg hover:bg-accent transition-colors w-full 
                    bg-gradient-to-r from-blue-500 to-purple-600 text-white 
                    transform hover:scale-105 hover:shadow-lg 
                    transition-all duration-300 ease-in-out`}
                >
                  <Pin 
                    className="absolute -top-2 -right-2 rotate-12 text-red-500" 
                    size={24} 
                    fill="currentColor" 
                    strokeWidth={1.5}
                  />
                  <Icon className="w-6 h-6 text-white" />
                  <span className="text-xs text-center w-full font-semibold">
                    {RouteIcons[path].name}
                  </span>
                </a>):(<Link
                  key={path}
                  href={path}
                  className={`flex items-center space-x-2 p-2 rounded-lg hover:bg-accent transition-colors hi there ${
...
>
  <Pin 
    className="absolute -top-5 -right-4 rotate-45 text-red-500" 
    size={36} 
    fill="currentColor" 
    strokeWidth={1.5}
  />
